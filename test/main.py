...
from src.gui import SearchGUI

# HOST = "http://localhost:9200"
# INDICES_NAMES = ["my_index_standart_analyzer", "my_index_standart_analyzer_ngramms", "my_index_custom_analyzer", "my_index_custom_analyzer_ngramms"]
...
INDICES_NAMES = [
    ...
    "my_index_standart_analyzer_semantic",
    "my_index_standart_analyzer_ngramms",
    "my_index_standart_analyzer_ngramms_semantic",
    "my_index_custom_analyzer",
    "my_index_custom_analyzer_semantic",
    "my_index_custom_analyzer_ngramms",
    "my_index_custom_analyzer_ngramms_semantic"
]
...
index_settings_standart_analyzer_semantic = {
    "settings": {
        "number_of_shards": 1,
        "number_of_replicas": 0,
        "analysis": {
...
index_settings_standart_analyzer_ngramms_semantic = {
    "settings": {
        "number_of_shards": 1,
        "number_of_replicas": 0,
        "index": {
...
index_settings_custom_analyzer_semantic = {
    "settings": {
        "number_of_shards": 1,
        "number_of_replicas": 0
    },
...
index_settings_custom_analyzer_ngramms_semantic = {
    "settings": {
        "number_of_shards": 1,
        "number_of_replicas": 0,
        "index": {
...
while True:
    ...
    if user_input.strip() == "ofoi":
        index_name = int(input("Введите номер индекса от 0 до 3\n>> "))
        field = input(
            "Введите поле, по которому будет производиться поиск\n>> "
        )
        ...
        sr.search_one_field_and_index(
            query=query,
            field=field,
            index_name=INDICES_NAMES[index_name],
            fuzziness=fuzziness,
            num_of_responses=num_of_responses
        )

    if user_input.strip() == "ofmi":
        field = input(
            "Введите поле, по которому будет производиться поиск\n>> "
        )
        num_of_responses = int(
        ...
        sr.search_one_and_many_indices(
            query=query,
            field=field,
            indices_names=INDICES_NAMES,
            fuzziness=fuzziness,
    ...
    if user_input.strip() == "mfoi":
        ...
        query = input("Введите ваш запрос\n>> ")
        sr.search_many_fields_one_index(
            query=query,
            fields=fields,
            index_name=INDICES_NAMES[index_name],
            fuzziness=fuzziness,
